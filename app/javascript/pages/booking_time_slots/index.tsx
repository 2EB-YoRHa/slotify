  ...
  return (
    <AppLayout>
      {!custom_time_slots_enabled && (
        <div className="mb-6 rounded-2xl border border-amber-100 bg-amber-50 p-4 sm:mb-8 sm:p-6">
          <div className="flex items-start gap-3 sm:gap-4">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white text-amber-500 shadow-sm sm:h-12 sm:w-12">
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[0.9fr_1.1fr] xl:gap-8">
        ...
        >
          <div className="mb-6 flex flex-col gap-4 sm:mb-7 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
            ...
            <div className="w-fit rounded-full bg-slate-50 px-4 py-2 text-xs font-extrabold uppercase tracking-wide text-slate-500">
              {booking_time_slots.length} saved
            </div>
          </div>

          {booking_time_slots.length === 0 ? (
            <div className="flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-5 py-10 text-center">
